...
from mobilenet_utils._internally_replaced_utils import load_state_dict_from_url
from mobilenet_utils.ops.misc import ConvNormActivation
from mobilenet_utils.utils import _log_api_usage_once
from mobilenet_utils._utils import _make_divisible
...
class MobileNetV2(nn.Module):
    def __init__(
        ...
        Args:
            ...
            states (int): the number of output regression states
            vel_layer_width (int): width of the velocity layer
        """
        super().__init__()
        ...
        self.fc_layer_with_vel = nn.Linear(self.last_channel + 1, vel_layer_width)
        self.relu = torch.nn.ReLU()

        self.fc_layer_end = nn.Linear(vel_layer_width, states)
    ...
    def _forward_impl(self, x: Tensor, v) -> Tensor:
        ...
        x = self.relu(self.fc_layer_with_vel(x))
        x = self.fc_layer_end(x)
        return x

    def forward(self, x: Tensor, v) -> Tensor:
        return self._forward_impl(x, v)
